...
double Surface::get_surface(vec n) {
  ...
  for (long j = 0; j < numbers; j++) {
    if (j == numbers - 1)
      surface += (vertex[j] ^ vertex[0]) * n;
    else
      surface += (vertex[j] ^ vertex[j + 1]) * n;
  }
  if (surface < 0)
    surface *= -1;
  return surface / 2;
...
void tetrahedron_circumcenter(
  ...
  // cout << "Ca_y" << ca_y << "Da_z "<<da_z << "Da_y "<< da_y << "Ca_z " <<
  // ca_z   << endl;
  ...
  /*
  denominator *= 2.0;
  *xi   = (circ_x * cross_cd_x + circ_y * cross_cd_y + circ_z * cross_cd_z) *
  denominator; *eta  = (circ_x * cross_db_x + circ_y * cross_db_y + circ_z *
  cross_db_z) * denominator; *zeta = (circ_x * cross_bc_x + circ_y * cross_bc_y
  + circ_z * cross_bc_z) * denominator;
*/
}
