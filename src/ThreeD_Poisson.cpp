...
using namespace Eigen;
  ...
  ThreeD::ThreeD(long N1, long N2, long N3, double a, double b, double c, double d, double e, double f,
...
Eigen::VectorXd ThreeD::solve1() {
    SparseMatrix<double> mat((N1+1)*(N2+1)*(N3+1), (N1+1)*(N2+1)*(N3+1));
    Eigen::VectorXd vec((N1+1)*(N2+1)*(N3+1));
    Eigen::VectorXd ergebnis((N1+1)*(N2+1)*(N3+1));
    vector<Triplet<double> > tr;
        //Dirichlet Randwerte
        //Für alle folgenden Schleifen, j ist für die x-Achse, i ist für die y-Achse, k ist für die z-Achse
         for(long k = 0; k < N3 + 1; k++) {
            for(long i = 0; i < N1+1; i++) {
                for(long j = 0; j < N2+1; j++) {
                    if(i == 0 || i == N1 || j == 0 || j == N2 || k == 0 || k == N3) {
                            vec[k*(N1+1)*(N2+1)+i*(N1+1)+j] = dirichlet(a+j*h1, c+ i*h2, e + k*h3);
                    }
                    else vec[k*(N1+1)*(N2+1)+i*(N1+1)+j] = (-1) * h1 * h2 * h3 * fun(a + j*h1, c + i*h2, e + k*h3);
                }
            }
        }
        for(long k = 0; k < N3 +1; k++) {
            for(long i = 0; i < N1+1; i++) {     //
                for(long j = 0; j < N2+1; j++) {
                    if(i == 0 || i == N1 || j == 0 || j == N2 || k == 0 || k == N3) {
                        mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,k*(N2+1)*(N1+1)+i*(N1+1)+j) = 1;
                    }
                    else {
                        mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,k*(N2+1)*(N1+1)+i*(N1+1)+j) = (-2)*(h2*h3)/h1 - (2*h1*h3)/h2 - (2*h1*h2)/h3;
                        //mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,k*(N2+1)*(N1+1)+i*(N1+1)+j) = ((-2)*h1*h3)/h2;
                        //mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,k*(N2+1)*(N1+1)+i*(N1+1)+j) = ((-2)*h1*h2)/h3;

                        mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,(k+1)*(N2+1)*(N1+1)+i*(N1+1)+j) = (h1*h2)/h3;
                        mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,(k-1)*(N2+1)*(N1+1)+i*(N1+1)+j) = (h1*h2)/h3;

                        mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,k*(N2+1)*(N1+1)+(i+1)*(N1+1)+j) = (h1*h3)/h2;
                        mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,k*(N2+1)*(N1+1)+(i-1)*(N1+1)+j) = (h1*h3)/h2;

                        mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,k*(N2+1)*(N1+1)+i*(N1+1)+j+1) = (h2*h3)/h1;
                        mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,k*(N2+1)*(N1+1)+i*(N1+1)+j-1) = (h2*h3)/h1;
                    }
                }
            }
...
Eigen::VectorXd ThreeD::solve2() {
    ...
    Eigen::VectorXd ergebnis((N1+1)*(N2+1)*(N3+1));
        //Dirichlet Randwerte
        //Für alle folgenden Schleifen, j ist für die x-Achse, i ist für die y-Achse, k - z-Achse
        for(long k = 0; k < N3 +1; k++) {
            for(long i = 0; i < N1+1; i++) {
                for(long j = 0; j < N2+1; j++) {
                    if(i == 0 || i == N1 || j == 0 || j == N2 || k == 0 || k == N3) {
                            vec[k*(N2+1)*(N1+1)+i*(N1+1)+j] = dirichlet(a+j*h1, c+ i*h2, e + k*h3);
                    }
                    else vec[k*(N2+1)*(N1+1)+i*(N1+1)+j] = (-1) * fun(a+j*h1, c+ i*h2, e + k*h3);
                }
            }
        }
        for(long k = 0; k < N3+1; k++) {
            for(long i = 0; i < N1+1; i++) {     //
                for(long j = 0; j < N2+1; j++) {
                    if(i == 0 || i == N1 || j == 0 || j == N2 || k == 0 || k == N3) {
                        ...
                        mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,k*(N2+1)*(N1+1)+i*(N1+1)+j)    = (-2)*(1/pow(h1,2) + 1/pow(h2,2) + 1/pow(h3,2));

                        mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,k*(N2+1)*(N1+1)+i*(N1+1)+(j+1)) = 1/pow(h1,2);
                        mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,k*(N2+1)*(N1+1)+i*(N1+1)+(j-1)) = 1/pow(h1,2);

                        mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,k*(N2+1)*(N1+1)+(i+1)*(N1+1)+j) = 1/pow(h2,2);
                        mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,k*(N2+1)*(N1+1)+(i-1)*(N1+1)+j) = 1/pow(h2,2);

                        mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,(k+1)*(N2+1)*(N1+1)+i*(N1+1)+j) = 1/pow(h3,2);
                        mat.insert(k*(N2+1)*(N1+1)+i*(N1+1)+j,(k-1)*(N2+1)*(N1+1)+i*(N1+1)+j) = 1/pow(h3,2);
                        /*
