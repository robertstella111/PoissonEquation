...
double maximumNorm(Eigen::VectorXd vec1, Eigen::VectorXd vec2, long n) {
    ...
    for(long i = 0; i < vec1.size(); i++) {
        if(vec1[i] > vec2[i]) {
            if(vec1[i]-vec2[i] > ergebnis) ergebnis = vec1[i]-vec2[i];
        }
        else {
            if(vec2[i]-vec1[i] > ergebnis) ergebnis = vec2[i]-vec1[i];
        }
    }
    return ergebnis;
}
...
double Ladungsverteilung2D(double x, double y) {
    return 2*sin(x)*cos(y);
}
double Dirichlet(double x, double y) {
    return sin(x)*cos(y);
}
double Neumannx(double x, double y) {
    return cos(x)*cos(y);
}
double Neumanny(double x, double y) {
...
double Ladungverteilung(double x) {
    if(0 <= x && x <= 0.2) return 0;
    else if( x <= 0.5) return 10e-11/epsilon;
    else if(x <= 0.8) return -10e-11/epsilon;
    else return 0;
}

double right(double t, double y) {
    return y + exp( t)*cos(t);
}

double richtig(double t) {
    return exp(t)*sin(t);
}
    ...
    for(long k = 0; k < N3 +1; k++) {
        for(long i = 0; i < N2+1; i++) {     //
            for(long j = 0; j < N+1; j++) {
                richtig[k*(N+1)*(N2+1)+i*(N+1)+j] = Dirichlet3D(a+j*h1, c + i *h2, e + k*h3);
            }
        }
    }
    //test.solvewitheFV();
