...
Eigen::VectorXd ONED::solve() {
    ...
    if((rand1 == 0 || rand1 ==1) && (rand2 == 0 || rand2 == 1)) {
        ...
        for(long i = 0; i < N+1; i++) {
            if(i == 0) vec[i] =  d1;
            else if(i == N) vec[i] = d2;
            else vec[i] = pow(h,2)*fun(a + i*h);
        }
        for (int k = 0; k < N+1; k++) {
           if(k == 0) mat.insert(k,k) = 1;
           else if(k == N) {
                mat.insert(k, k) = 1;
           }
           else {
        ...
        Eigen::VectorXd ergebnis(N+1);
        for(long i = 0; i < N+1; i++) {
            if(i == 0) vec[i] = -fun(a + i*h) + 2*n1/h;
            else if(i == N) vec[i] = -fun(a+i*h) - 2 * n2/h;
            else vec[i] = pow(h,2)*fun(a + i*h);
        }
        for (int k = 0; k < N+1; k++) {
            if(k == 0)  {
        ...
        //NxN Matrix, Bei einem Rand handelt es sich um Dirichlet beim anderen um Neumann Bedingungen
        SparseMatrix<double> mat (N+1,N+1);
        Eigen::VectorXd vec(N+1);
        Eigen::VectorXd ergebnis(N+1);
        ...
        if(rand1 == 2) {
            ...
            for(long i = 0; i < N+1; i++) {
                if(i == 0) vec[i] = d1;
                else if(i == N) vec[i] =  vec[i] = -fun(a+i*h) - 2 * n2/h;
                else vec[i] = pow(h,2)*fun(a + i*h);
            }
            for (int k = 0; k < N+1; k++) {
                if(k == 0)  {
